...
class DataPrePro:
    def __init__(self):
        ...
        self.feature_selection: Optional[FeatureSelectionAlgorithm] = None
    ...
    def register_routes(self):
        ...
        def handle_merge_data():
            ...
            left_on = [left_row_data[i] for i in range(len(left_row_data)) if (left_row_data[i] != '空' and right_row_data[i] != '空')]
            right_on = [right_row_data[i] for i in range(len(right_row_data)) if (left_row_data[i] != '空' and right_row_data[i] != '空')]
            for index in range(len(left_on)):
                if self.right_cleaner and self.left_cleaner and self.left_cleaner.data[left_on[index]].dtype == self.right_cleaner.data[right_on[index]]: # noqa
                    continue
                else:
                    return jsonify({'success': False, 'message': '请正确完成合并依据'})
            try:
                self.mergeData = self.merge_data(self.left_cleaner.data, self.right_cleaner.data, left_on=left_on, right_on=right_on)
        ...
        def feature_selection():
            ...
            selected_file = request.form.get('selected_file')
            if selected_file:
                self.feature_selection = FeatureSelectionAlgorithm(file_path=selected_file)
                self.feature_selection.loadfile()
                all_columns = self.feature_selection.get_columns()
                if len(all_columns) == len(set(all_columns)):
        ...
        def feature_selection_step():
            ...
            if not len(response):
                flash('请选择目标列')
                return render_template('feature_selection.html', step='select_column', columns=all_columns)
            self.feature_selection.exchange(response)
            columns = self.feature_selection.get_columns()
            self.feature_selection.normalization()
            parameter_omega = 0.66
            label_length = len(response)
            result = self.feature_selection.RankFeaturesBasedOnLabelDistributionUsingNeigborhoodMutualInformation(label_length
                                                                                                                  , parameter_omega)
            feature_sort = {}
            for column, sequence in zip(columns, result):
                feature_sort[column] = sequence
            feature_sort = dict(sorted(feature_sort.items(), key=lambda x: x[1], reverse=True))
            return render_template('feature_selection.html', step='select_column',
                                   feature_sort=feature_sort,columns=columns, selected_columns=response)

        @self.app.route('/download_results')
